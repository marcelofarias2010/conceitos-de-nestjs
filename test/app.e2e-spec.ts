...
describe('PessoasController (e2e)', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [
        // LINHA REMOVIDA: ConfigModule.forFeature(globalConfig),
        // Mantemos o GlobalConfigModule pois ele existe na sua pasta
        GlobalConfigModule,
        ConfigModule.forRoot({
          // Adicione isso se precisar carregar variáveis de ambiente .env no teste
          isGlobal: true,
          envFilePath: '.env',
        }),
  ...
  describe('PATCH /pessoas/:id', () => {
    it('deve atualizar uma pessoa', async () => {
      ...
      const authToken = await login(app, 'joao2@email.com', '123456');
  ...
  describe('DELETE /pessoas/:id', () => {
    it('deve remover uma pessoa', async () => {
      ...
      expect(response.body.email).toBe('joao2@email.com');
    });
